...
export const API_BASE: string =
  ...
  trimSlash(((Constants.expoConfig?.extra as any)?.API_BASE_URL as string)) ||
...
api.interceptors.response.use(
  ...
  async (err: AxiosError<any>) => {
    ...
    console.log('API ERROR →', method, url, '→', status ?? err.code ?? err.message, body ?? '');
...
export async function getUserPublic(userId: string): Promise<Partial<UserMe> & { id: string }> {
  const { data } = await api.get(`/users/${userId}`);
  return data;
}

export async function updateAccountSettings(patch: Partial<UserMe>): Promise<UserMe> {
  return tryJson(
...
export async function uploadAvatar(imageUri: string): Promise<{ url: string }> {
  ...
  try {
    ...
    if (String(e?.message || '').includes('(404)')) {
      try { return await doUpload('/auth/me/avatar-binary'); } catch (_) {}
      return await doUpload('/auth/avatar-binary'); // letzter Alias
    }
    throw e;
  }
}
...
export async function fetchTrendingTags(limit = 16): Promise<{ tag: string; count: number }[]> {
...
export async function likeComment(postId: string, commentId: string) {
  try { const { data } = await api.post(`/posts/${postId}/comments/${commentId}/like`); return data; }
  catch { return {}; }
}
export async function unlikeComment(postId: string, commentId: string) {
  try { const { data } = await api.post(`/posts/${postId}/comments/${commentId}/unlike`); return data; }
  catch { return {}; }
}
...
export async function createPost(input: {
  ...
  if (result.status >= 200 && result.status < 300) {
    ...
    catch { return { ok: true, raw: result.body }; }
  }

  let msg = result.body;
  try { const p = JSON.parse(result.body || '{}'); msg = p?.message || p?.error || result.body; } catch {}
...
export async function chatSearchUsers(q: string): Promise<any[]> {
  ...
  try { return ((await api.get('/users/search', { params })).data?.users ?? []) as any[]; } catch {}
  try { return ((await api.get('/search/users', { params })).data?.users ?? []) as any[]; } catch {}
  return [];
}
...
export async function chatGetMessages(
  ...
  try {
    ...
    const { data } = await api.get(`/chat/threads/${chatId}/messages`, { params });
    return data as { messages: ChatMessage[]; nextCursor: number | null };
  }
}
...
export async function chatDelete(chatId: string): Promise<void> {
  try {
    ...
    catch { await api.delete(`/chat/threads/${chatId}`); }
  }
}
...
export async function groupLeave(chatId: string): Promise<void> {
  try {
    ...
    try { await api.post(`/chat/${chatId}/leave`); }
    catch { await api.post(`/chat/threads/${chatId}/leave`); }
  }
}