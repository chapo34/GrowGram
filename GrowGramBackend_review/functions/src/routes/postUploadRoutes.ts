...
import formidable, { File as FFile, Fields, Files } from 'formidable';
...
router.post('/upload', async (req, res) => {
  try {
    ...
    const visibility =
      (firstVal(fields['visibility']) ?? 'public').toLowerCase() === 'private' ? 'private' : 'public';
    const folder = sanitizeFolder(firstVal(fields['folder']) ?? 'uploads');

    const srcBuf = await readFile(fileAny.filepath);
    const mime = fileAny.mimetype || 'application/octet-stream';
    const { clean, outMime } = await sanitizeImageBuf(srcBuf, mime);

    const orig = fileAny.originalFilename || 'image';
    const ext  = (outMime.split('/')[1] || 'jpg').toLowerCase();
    const safeName = (orig.replace(/[^\w.\-]+/g, '_') || `image.${ext}`).replace(/\.(jpe?g|png|webp|heic|heif)$/i, `.${ext}`);
    ...
    const publicUrl =
      `https://firebasestorage.googleapis.com/v0/b/${gcsBucket.name}/o/${encodeURIComponent(destPath)}?alt=media&token=${dlToken}`;
    ...
    await docRef.set({
      ...
      text,
      mediaUrls: [publicUrl],
      tags, tagsLower, visibility,
      likesCount: 0, commentsCount: 0, deleted: false, score: 0,
...
router.post('/upload-binary', async (req, res) => {
  try {
    ...
    let tags: string[] = [];
    if (q.tags) {
      try { const parsed = JSON.parse(q.tags); if (Array.isArray(parsed)) tags = parsed.map(String); }
      catch {}
    }
    tags = uniq(tags).slice(0, 10);
    const tagsLower = tags.map(norm);
