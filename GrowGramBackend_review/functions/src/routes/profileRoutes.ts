...
router.get('/me', async (req, res) => {
  try {
    ...
    return res.json(await readUser(uid));
  } catch (e: any) {
    return res.status(e?.status || 500).json({ error: 'me_failed', details: String(e?.message || e) });
  }
});
...
router.patch('/me', async (req, res) => {
  try {
    ...
    return res.status(e?.status || 500).json({ error: 'update_failed', details: String(e?.message || e) });
  }
});

// POST /me/avatar-binary    (Body: image/*)
router.post('/me/avatar-binary', async (req, res) => {
  try {
    const uid = getUid(req);
    ...
    return res.status(e?.status || 500).json({ error: 'avatar_failed', details: String(e?.message || e) });
  }
});
