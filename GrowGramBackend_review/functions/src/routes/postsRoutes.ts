...
function getUserId(req: any): string {
  // Produktions-Setup: aus JWT (req.user.uid o.ä.). Für Tests erlauben wir x-user-id.
...
router.post('/:postId/like', async (req, res) => {
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      likesCount = prev + 1;
      tx.update(postRef, { likesCount, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
    });
    return ok(res, { likesCount });
  } catch (e: any) {
    return res.status(400).json({ ok: false, message: 'post_like_failed', details: String(e.message || e) });
  }
});
...
router.post('/:postId/unlike', async (req, res) => {
  try {
    ...
    return res.status(400).json({ ok: false, message: 'post_unlike_failed', details: String(e.message || e) });
  }
});
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (!saveSnap.exists) {
        ...
        savesCount = prev + 1;
        tx.update(postRef, { savesCount, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
        saved = true;
      } else {
        // schon gespeichert → idempotent
        saved = true;
    ...
    return res.status(400).json({ ok: false, message: 'post_save_failed', details: String(e.message || e) });
  }
});
...
router.post('/:postId/unsave', async (req, res) => {
  try {
    ...
    return res.status(400).json({ ok: false, message: 'post_unsave_failed', details: String(e.message || e) });
  }
});
...
router.get('/:postId/comments', async (req, res) => {
  try {
    ...
    return res.status(400).json({ ok: false, message: 'comments_fetch_failed', details: String(e.message || e) });
  }
});
...
router.post('/:postId/comments', async (req, res) => {
  try {
    ...
    const comment = {
      ...
      postId,
      text,
      author: { name: 'Du' }, // später: aus Auth
      likesCount: 0,
      liked: false,
      createdAt: new Date().toISOString(), // für Client – zusätzlich serverTimestamp halten
      _serverCreatedAt: now,
    };
    ...
    return res.status(400).json({ ok: false, message: 'comment_add_failed', details: String(e.message || e) });
  }
});
...
router.post('/:postId/comments/:commentId/like', async (req, res) => {
  try {
    ...
    return res.status(400).json({ ok: false, message: 'comment_like_failed', details: String(e.message || e) });
  }
});
...
router.post('/:postId/comments/:commentId/unlike', async (req, res) => {
  try {
    ...
    return res.status(400).json({ ok: false, message: 'comment_unlike_failed', details: String(e.message || e) });
  }
});

// Optionale negative Signal-Route (z. B. Long-press „Nicht interessiert“)
router.post('/:postId/hide', async (req, res) => {
  try {
    ...
    return res.status(400).json({ ok: false, message: 'hide_signal_failed', details: String(e.message || e) });
  }
});
