...
function parseMultipart(req: any, res: any): Promise<void> {
  return new Promise((resolve, reject) => {
    const handler = upload.fields([{ name: 'image', maxCount: 1 }, { name: 'file', maxCount: 1 }]) as any;
...
export async function postUpload(req: Request, res: Response) {
  try {
    ...
    const uid = (payload as any).userId || (payload as any).uid || (payload as any).id;
    ...
    const file: Express.Multer.File | undefined = files?.image?.[0] || files?.file?.[0] || (req as any).file;
    ...
    const visibility =
      String((req.body as any)?.visibility || 'public').toLowerCase() === 'private' ? 'private' : 'public';
    ...
    const docRef = await db.collection('posts').add({
      ...
      updatedAt: now,
      credit: null,
      meta: { mime: file.mimetype, size: file.size, filename: safeName },
    });

    return res.status(201).json({ ok: true, id: docRef.id, url, visibility });
  } catch (e: any) {
    const code =
      e instanceof MulterError ? 400 :
      e?.message === 'unsupported_file_type' ? 415 :
      e?.status || 500;
