// src/features/feed/screens/ExploreScreen.tsx
...
export default function ExploreScreen() {
  ...
  const loadMore = useCallback(async () => {
    ...
    if (q.trim().length >= 2) {
      ...
      try {
        setLoadingMore(true);
        const { posts, nextCursor } = await searchPostsPage(
          q.trim(),
          20,
          sCursor,
          activeTag ?? undefined
    ...
    try {
      setLoadingMore(true);
      const { posts, nextCursor } = await fetchTrendingPage(
        20,
        cursor ?? undefined,
        activeTag ?? undefined
      );
      ...
      if (posts?.length) {
        setItems((prev) => [
          ...prev,
          ...posts.map((p: FeedPost) => ({ ...p, _liked: false })),
        ]);
      }
  ...
  const selectTag = useCallback(
    (t: string | null) => {
      setActiveTag(t);
      if (q.trim().length >= 2) {
        doServerSearch(q.trim(), t ?? undefined);
      } else {
        loadFirst(t);
      }
    },
        ...
        arr.map((p) =>
          p.id !== postId
            ...
            : {
                ...
                likesCount: Math.max(
                  0,
                  (p.likesCount ?? 0) + (intendedLike ? 1 : -1)
                ),
              }
        );
  ...
  const updateCommentCount = useCallback(
    (count: number) => {
      if (!cPostId) return;
      const apply = (arr: FeedPost[]) =>
        arr.map((p) => (p.id === cPostId ? { ...p, commentsCount: count } : p));
      if (q.trim().length >= 2) setSItems((prev) => apply(prev));
      else setItems((prev) => apply(prev));
    },
    [cPostId, q]
              ...
              >
                ...
                >
                  <Text
                    style={[
                      styles.chipTxt,
                      !activeTag && styles.chipTxtActive,
                    ]}
                  >
                    Alle
                  </Text>
    ...
    ({ item }: ListRenderItemInfo<FeedPost>) => {
      ...
      return (
        <PostCard
          ...
          onPress={() => nav.navigate('Post', { post: item })}
        />
      );
    },
  ...
  const listFooter = useMemo(() => {
    ...
    if (q.trim().length < 2 && !cursor) {
      return (
        <View style={{ paddingVertical: 28 }}>
          <Text style={{ color: MUTED, textAlign: 'center' }}>— Ende erreicht —</Text>
        </View>
      );
    }
    if (q.trim().length >= 2 && !sCursor) {
      return (
        <View style={{ paddingVertical: 28 }}>
          <Text style={{ color: MUTED, textAlign: 'center' }}>— Ende erreicht —</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  chipActive: {
    ...
    borderColor: 'rgba(168,255,176,0.35)',
  },
