...
import { View, Text, StyleSheet, Pressable, Dimensions, Platform, ActivityIndicator, Animated } from 'react-native';
...
const INTRO_STING = require('../../../../assets/intro/sting.wav');const INTRO_DURATION_MS = 4000;
...
export default function IntroScreen() {
  ...
  const getRouteIfExists = useCallback((name: string): string | null => {
    try {
  ...
  const resolveDefaultTarget = useCallback((): string | null => {
  ...
  const replaceTo = useCallback((name: string) => {
    try {
      (navigation as any).reset?.({ index: 0, routes: [{ name }] });
      return;
    } catch {}
  ...
  const readLocalComplianceAck = useCallback(async () => {
    try {
      ...
      const userId: string | null = rawUser ? (JSON.parse(rawUser)?.id ?? null) : null;
      if (!userId) return { hasAck: false, userId: null };
      const key = `${STORAGE_KEYS.COMPLIANCE_PREFIX}${userId}`;
      const v = await AsyncStorage.getItem(key);
      let hasAck = v === '1';
      if (!hasAck && v) {
        try { const j = JSON.parse(v); hasAck = j?.agreed === true || j?.accepted === true; } catch {}
      }
      return { hasAck, userId };
    } catch { return { hasAck: false, userId: null }; }
  }, []);
  ...
  const goNext = useCallback(async () => {
    ...
    if (!token) { replaceTo(getRouteIfExists('Auth') || 'Auth'); return; }

    const { hasAck } = await readLocalComplianceAck();
    if (!hasAck) { replaceTo(getRouteIfExists('WelcomeCompliance') || 'WelcomeCompliance'); return; }

    replaceTo(getRouteIfExists('Main') || resolveDefaultTarget() || 'Main');
  }, [readLocalComplianceAck, getRouteIfExists, replaceTo, resolveDefaultTarget]);
  ...
  const finish = useCallback(async () => {
    ...
    await fadeAudioOut();
    Animated.timing(fadeRef.current, { toValue: 0, duration: 260, useNativeDriver: false }).start(() => { void goNext(); });
  }, [fadeAudioOut, goNext]);
  ...
  const playSting = useCallback(async () => {
    ...
    try {
      const { sound } = await Audio.Sound.createAsync(INTRO_STING, { volume: 1, shouldPlay: true });
      stingRef.current = sound;
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    } catch {}
  }, []);

  const onPlaybackUpdate = useCallback((st: AVPlaybackStatus) => {
    if (!st.isLoaded) return;
    if (!hasStarted && (st.positionMillis ?? 0) > 50) {
      setHasStarted(true);
      scheduleFinish();
      if (tSting.current) clearTimeout(tSting.current);
      tSting.current = setTimeout(playSting, Math.max(0, STING_AT_MS - (st.positionMillis ?? 0)));
    }
  ...
  const onErr = useCallback(() => { setErr('Intro konnte nicht geladen werden.'); setTimeout(finish, 500); }, [finish]);
  ...
  return (
    <Animated.View style={[styles.container, { opacity: fadeRef.current }]}>
      ...
      <LinearGradient colors={['transparent', 'rgba(0,0,0,0.25)', 'rgba(0,0,0,0.6)']} style={styles.overlayBottom} pointerEvents="none" />
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Intro überspringen" onPress={finish} style={({ pressed }) => [styles.skipBtn, pressed && { opacity: 0.6 }]}>
      ...
      {__DEV__ && Platform.select({ ios: true, android: true }) && (
        <View style={styles.devHint}><Text style={styles.devHintTxt}>Intro • Tippen = Ton erzwingen • Skip = sofort weiter</Text></View>
      )}
    </Animated.View>
  );
}
...
const styles = StyleSheet.create({
  container: { width: W, height: H, backgroundColor: '#000', alignItems: 'center', justifyContent: 'center' },
  videoWrap: { width: W, height: H, alignItems: 'center', justifyContent: 'center', backgroundColor: '#000' },
  video: { width: W, height: H },
  overlayBottom: { position: 'absolute', bottom: 0, width: W, height: H * 0.42 },
  brandWrap: { position: 'absolute', bottom: H * 0.12, width: W, alignItems: 'center', justifyContent: 'center' },
  brandTitle: { color: '#fff', fontSize: 40, fontWeight: '800', letterSpacing: 0.5, textShadowColor: 'rgba(76,175,80,0.9)', textShadowOffset: { width: 0, height: 0 }, textShadowRadius: 16 },
  ...
  devHint: { position: 'absolute', bottom: 12, left: 0, right: 0, alignItems: 'center' },
  devHintTxt: { color: 'rgba(255,255,255,0.6)', fontSize: 12 },
});