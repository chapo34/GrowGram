...
export type ThemeColors = {
  ...
  /** Standard-Border-Farbe für Inputs / Karten / Outlines */
  border: string;
...
export const DARK_COLORS: Omit<
  ThemeColors,
  'accent' | 'accentFg' | 'buttonGradient' | 'card'
> & {
  card?: string;
} = {
  ...
  // Standard-Border im Dark Mode (sehr dezent)
  border: 'rgba(170,255,210,0.14)',

  // Accents (werden in ThemeProvider final gesetzt)
  gold: '#E9C86C',
...
export const LIGHT_COLORS: Omit<
  ThemeColors,
  'accent' | 'accentFg' | 'buttonGradient' | 'card'
> & {
  card?: string;
