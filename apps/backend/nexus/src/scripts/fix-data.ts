// backend/nexus/src/scripts/fix-data.ts
/**
...
function parseFlags(): Flags {
  ...
  const argv = idx >= 0 ? process.argv.slice(idx + 1).filter(Boolean) : [];
  ...
  for (const doc of snap.docs) {
    ...
    if (d.compliance && typeof d.compliance === 'object') {
      patch.compliance = {
        ...
        agreedAt: d.compliance.agreedAt || FieldValue.serverTimestamp(),
      };
    }

    // Timestamps
    if (!d.createdAt) patch.createdAt = FieldValue.serverTimestamp();
