...
export const indexPost = functions
  ...
  .onWrite(async (chg, _ctx) => {
    ...
    const sameTags =
      tagsBefore.length === tagsAfter.length && tagsBefore.every((t: string, i: number) => t === tagsAfter[i]);
    ...
    for (const raw of uniq([...wordsNoHash, ...tagsAfter])) {
      const w = raw.replace(/^#/, '');
    ...
    // 2) Mehrwortige Strains (3-grams, 2-grams)
    const grams = [...ngrams(wordsNoHash, 3), ...ngrams(wordsNoHash, 2)];
