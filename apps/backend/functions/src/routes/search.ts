...
function resolveQuery(qRaw: string): { kind: 'strain'|'tag'|'free'; value?: string } {
  ...
  const toks = tokenize(q).map(t => t.replace(/^#/, ''));
  ...
  if (toks.length === 1 && SPECIES_ALIASES[toks[0]]) return { kind: 'tag', value: SPECIES_ALIASES[toks[0]] };
  if (toks.length === 1 && FAMILY_ALIASES[toks[0]])  return { kind: 'tag', value: FAMILY_ALIASES[toks[0]] };
  if (toks.length === 1 && CANON_TAG_ALIASES[toks[0]]) return { kind: 'tag', value: CANON_TAG_ALIASES[toks[0]] };
  return { kind: 'free' };
}
...
/** GET /feed/search/suggestions?q=…&limit=8 */
